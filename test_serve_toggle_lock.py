"""Non-régression du blocage de /api/toggle par le verrou inter-processus
« registre » (AUDIT-2026-08-13.md, section 28.33) : la réponse HTTP ne doit
plus attendre ce verrou, seulement le travail qu'il protège en profite."""
...
class TestToggleNAttendPasLeVerrouRegistre(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def construire_handler(self):
        ...
        handler.reassembler = lambda identity: None  # hors sujet ici (28.32/28.9)
        return handler
    ...
    def test_la_reponse_ne_bloque_plus_sur_le_verrou_tenu_ailleurs(self):
        ...
        try:
            ...
            reponses = self.appeler_toggle(handler, excluded=True)
    ...
    def test_identifiant_invalide_repond_tout_de_suite_sans_verrou(self):
        handler = self.construire_handler()
        ...
        handler.send_json = lambda payload, code=200: reponses.append((code, payload))
        handler.do_POST()
        self.assertEqual(reponses, [(400, {"error": "identifiant invalide"})])
